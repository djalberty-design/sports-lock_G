...
import { applyNflToMeans } from "./nfl-matchup.ts";`nimport { applyMlbParkToMeans } from "./mlb-park.ts";
...
export function buildLatents(input: ChanceInput & { eventId: string; chanceHome?: number } & LiveLatentFields): {
  ...
  chaos = Math.min(0.28, chaos + avail.chaosAdd + officialMeans.chaosAdd + matchupMeans.chaosAdd + mlbParkMeans.chaosAdd);
