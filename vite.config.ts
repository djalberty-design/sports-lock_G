...
/** Local vite-preview loads PGLite from the server bundle; copy wasm/data next to it. */
  ...
  return {
    ...
    closeBundle() {
      ...
      const dests = [
        ...
        join(process.cwd(), "dist/server/_libs"),
      ];
      for (const dest of dests) {
        const parent = join(dest, "..");
        if (!existsSync(parent)) continue;
        mkdirSync(dest, { recursive: true });
        for (const file of ["pglite.data", "pglite.wasm", "initdb.wasm", "initdb.js"]) {
